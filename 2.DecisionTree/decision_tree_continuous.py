...
# dataset: https://alliance.seas.upenn.edu/~cis520/wiki/index.php?n=Lectures.DecisionTrees
...
def info_gain_continue(x_i, y):
    ...
    for thred in x_i_sorted[:-1]:
        ...
        new_ent = p * entropy(y[x_i >= thred]) + (1-p) * entropy(y[x_i < thred])
...
def recursive_split(x, y):
    ...
    if np.all(gains < 1e-3) or gains[selected_attr_idx, 0] < 1e-6:    return y
    ...
    if is_discrete[selected_attr_idx]:
        ...
        split_sets = partition_continue(x[:, selected_attr_idx], gains[selected_attr_idx, 1])

    res = {}
    for k, v in split_sets.items():
        # split data into sub data set
        x_sub, y_sub = x.take(v, axis=0), y.take(v, axis=0)
        res["x_%d = %s" % (selected_attr_idx, k)] = recursive_split(x_sub, y_sub)
    return res


data = create_data()
x, y = data[:, 1:], data[:, 0]
# whether data in column is discrete, if is_discrete[i] == True, column i in x is discrete
is_discrete = [True, False, False, False, False, False, True]

print('Test for entropy:', entropy([1, 2]))
print('Test for entropy:', entropy([1, 1]))

print('Test for info_gian:', info_gain_continue(np.array([1, 2, 3, 4]), np.array([0, 0, 1, 1])))
print('Test for info_gian:', info_gain_continue(np.array([1, 2, 3, 4]), np.array([0, 1, 1, 1])))

print('Test for partition:', partition_continue([0.1, 0.5, 0.6, 0.3, 0.5, 0.2, 0.1], 0.3))
